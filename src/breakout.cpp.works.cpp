...
bool BreakOut::CheckBallAgainstBlock(game_info* gi, float t, float &cur_t, int &type_t, int last_type, float x, float y, int isPaddle)
{
	...
	if (chk < cur_t && (3 + (isPaddle * 4)) != last_type && gi->ball_dx > 0)
	{
		cur_t = chk;
		type_t = (3 + (isPaddle * 4));
	}
	...
	// check right edge
	chk = CheckBallAgainst(gi, t, (float)(x + 0.5), (float)(y - 0.25), (float)(x + 0.5), (float)(y-0.5) - 0.25);
	if (chk < cur_t && (5 + (isPaddle * 4)) != last_type && gi->ball_dx < 0)
	{
		cur_t = chk;
		type_t = (5 + (isPaddle * 4));
	}
	...
	if (chk < cur_t && (6 + (isPaddle * 4)) != last_type && gi->ball_dy > 0)
	{
		cur_t = chk;
		type_t = (6 + (isPaddle * 4));
	}

	if (type_t >= (3 + (isPaddle * 4)) && type_t <= (6 + (isPaddle * 4)))
	{
		return true;
	}
...
void BreakOut::MoveBall(game_info* gi, float t, int last_type)
{
	...
	if (gi->ball_dy > 0 && up_t < cur_t && 0 != last_type)
	{
		cur_t = up_t;
		type_t = 0;
	}

	if (gi->ball_dx > 0 && right_t < cur_t && 1 != last_type)
	{
		cur_t = right_t;
		type_t = 1;
	}

	if (gi->ball_dx < 0 && left_t < cur_t && 2 != last_type)
	{
		cur_t = left_t;
		type_t = 2;
	}
